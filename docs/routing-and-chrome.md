...
**Taxonomy breaks it, and returns `null` instead of a guess.** Hugo generates `/tags/<term>/`
from the term string, and terms are translated along with the posts:
`/tags/paruline-flamboyante/` and `/en/tags/american-redstart/` are the same tag on the same
post. A prefix swap there yields a URL that has never existed —
`netlify.toml:227` carries a redirect for exactly that pair, so this is observed, not
hypothetical. Mapping them needs a FR↔EN table derived from post membership, which the content
migration never produced; taxonomy is not represented under `web/content/` at all.

When the resolver returns `null` the switcher falls back to the target locale's homepage. The
resolver itself does not apply that fallback, so "no counterpart exists" stays distinguishable
from "the counterpart is the homepage".
...
- 14 `resolveCounterpart` cases pass, including deep blog URLs, both directions, taxonomy →
  `null`, and `/environnement/` (a path starting with the letters "en", which must not be
...
- **Taxonomy has no routes and no counterpart map.** 254 term pages plus their pagination
  exist on the live site and in Google's index; nothing here serves them yet. This is the
  largest single piece of unbuilt surface and is not tracked by any board item.
- Hugo emits **268 `page/1/` alias stubs** and a `/fr/` stub (all `noindex` meta-refresh).
  Nothing reproduces them. `netlify.toml` covers `/fr/*`; `page/1/` is unhandled.
