...
- **Taxonomy has no routes.** 254 term pages plus their pagination exist on the live site and
  in Google's index; nothing here serves them yet. This is the largest single piece of unbuilt
  surface. The *counterpart map* is no longer a gap — terms pair by prefix swap (above), and
  `lib/content/site-index.ts` already derives the full term set with both languages' URLs.
  Wiring the routes themselves is board item #16.
